...
function buildHomepageHTML(releasesJson, jckJSON, OS) {
  ...
  if (matchingFile) {
    if (matchingFile.binary.installer) {
      dlLatest.href = `./download.html?link=${matchingFile.binary.installer.link}`; // set the main download button's link to be the installer's download url
    } else {
      dlLatest.href = `./download.html?link=${matchingFile.binary.package.link}`; // set the main download button's link to be the binary's download url
